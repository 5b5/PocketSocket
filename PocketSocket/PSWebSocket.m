        ...
        if(_readyState == PSWebSocketReadyStateOpen &&
           ...
           [_delegate respondsToSelector: @selector(webSocketIsHungry:)]) {
            [self executeDelegate:^{
                [self->_delegate webSocketIsHungry: self];
            }];
        }
    } while (_outputStream.hasSpaceAvailable && _outputBuffer.hasBytesAvailable);
    _pumpingOutput = NO;
...
- (void) setReadPaused: (BOOL)paused {
    [self executeWork: ^{
        if (paused != self->_readPaused) {
            self->_readPaused = paused;
            if (!paused)
                [self pumpInput];
        }
    }];
...
- (void) askDelegateToValidateServerTrust: (NSStream*)stream {
    ...
    CFRetain(trust);
    [self executeDelegate:^{
        BOOL ok = [self->_delegate webSocket: self validateServerTrust: trust];
        CFRelease(trust);
        [self executeWork:^{
            if (ok) {
                self->_serverUnvalidated = NO;
                [self pumpOutput];
                [self pumpInput];
            } else {
                NSDictionary* userInfo = @{NSURLErrorFailingURLStringErrorKey: self->_request.URL.absoluteString};
